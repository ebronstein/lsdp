            ...
            for i in range(n_obs - self.n_obs_history - self.n_pred_horizon + 1):
                ...
                for key, value in obs.items():
                    ...
                    pred_horizon[key] = value[
                        i + self.n_obs_history : i + self.n_obs_history + self.n_pred_horizon
                    ]
...
class Diffusion(object):
    def __init__(
        ...
        if self.use_ema_helper:
            ...
            # self.ema_helper = EMAHelper(mu=0.999)
...
def _noise(x, eps=None, t=None):
    if t is None:
        t = torch.rand(x.shape[0], dtype=torch.float32, device=x.device)
    if eps is None:
        eps = torch.randn_like(x)
    alpha_t, sigma_t = _get_alpha_sigma(_expand(t, data_shape=x.shape[1:]))
    # print("t:", t.shape)
...
def sample(
        ...
        x_shape = (n,) + tuple(data_shape)
        x = torch.randn(x_shape, dtype=torch.float32).cuda()
        for i in range(num_steps):
            # breakpoint()
            t_cur = torch.full((n,), ts[i], dtype=torch.float32).cuda()
            t_next = torch.full((n,), ts[i + 1], dtype=torch.float32).cuda()

            alpha_cur, sigma_cur = _get_alpha_sigma(_expand(t_cur, tuple(data_shape)))
            alpha_next, sigma_next = _get_alpha_sigma(_expand(t_next, tuple(data_shape)))
            ddim_sigma = (sigma_next / sigma_cur) * torch.sqrt(
                1 - alpha_cur**2 / alpha_next**2
            )

            if cfg_val is None:
                eps_hat = model.model_fn(x, None, t_cur)
            else:
                eps_hat_cond = model.model_fn(x, t_cur)
                eps_hat_uncond = model.model_fn(x, t_cur, dropout_cond=True)
                eps_hat = eps_hat_uncond + cfg_val * (eps_hat_cond - eps_hat_uncond)

            x_hat = _x_hat(x, eps_hat, t_cur, tuple(data_shape))
            if clip_denoised:
...
if __name__=="__main__":
    path = "/home/tsadja/lsdp/diffusion_policy/data/pusht/pusht_cchi_v7_replay.zarr"
    ...
    for num_steps in num_steps_list:
        normalized_samples.append(
            sample(
                diffusion, num_samples, num_steps=num_steps, data_shape=(n_obs_history, 5), 
                clip_denoised=True, clip_noise=clip_noise
            )
            .cpu()
            .numpy()
        )
    ...
    print(np.isnan(normalized_samples).sum())
